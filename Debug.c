...
static int DAsm(char *S,word A)
{
    ...
    switch(Ads[OP+1]) {
    case Ac:
        sprintf(S,"%s a",Ops[Ads[OP]]);
        break;
    case Il:
        sprintf(S,"%s",Ops[Ads[OP]]);
        break;

    case Rl:
        J=Rd6502(B++);
        TO=A+2+((J<0x80)? J:(J-256));
        sprintf(S,"%s $%04X",Ops[Ads[OP]],TO);
        break;

    case Im:
        sprintf(S,"%s #$%02X",Ops[Ads[OP]],Rd6502(B++));
        break;
    case Zp:
        sprintf(S,"%s $%02X",Ops[Ads[OP]],Rd6502(B++));
        break;
    case Zx:
        sprintf(S,"%s $%02X,x",Ops[Ads[OP]],Rd6502(B++));
        break;
    case Zy:
        sprintf(S,"%s $%02X,y",Ops[Ads[OP]],Rd6502(B++));
        break;
    case Ix:
        sprintf(S,"%s ($%02X,x)",Ops[Ads[OP]],Rd6502(B++));
        break;
    ...
    case Ab:
        sprintf(S,"%s $%04X",Ops[Ads[OP]],RDWORD(B));
        B+=2;
        break;
    case Ax:
        sprintf(S,"%s $%04X,x",Ops[Ads[OP]],RDWORD(B));
        B+=2;
        break;
    case Ay:
        sprintf(S,"%s $%04X,y",Ops[Ads[OP]],RDWORD(B));
        B+=2;
        break;
    case In:
        sprintf(S,"%s ($%04X)",Ops[Ads[OP]],RDWORD(B));
