...
void RefreshSprites(register TMS9918 *VDP,register byte Y)
{
    ...
    for(L=0; L<32; ++L)
    {
        ...
        if((Y>K)&&(Y<=K+OH))
        {
            ...
            {
                /* Record extra sprite number in the VDP status register */
                VDP->Status=(VDP->Status&0x80)|TMS9918_STAT_5THSPR|(L&TMS9918_STAT_5THNUM);

                /* This is in place of a proper CS detection routine */
                VDP->Status|=TMS9918_STAT_OVRLAP;
                break;
            }
            /* Mark sprite as ready to draw */
            M|=1;
...
#endif // VTech Video Diagnostic Cart modifications
